...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000","https://doctor-dashboard-lyart.vercel.app/"],  # Add your Next.js app URL
...
async def get_symptoms():
    return {"symptoms": list(utils.dataset_to_friendly.values())}
...
async def predict_disease(input_data: SymptomInput):
    try:
        ...
        # Convert user-friendly symptoms to dataset format
        dataset_symptoms = []
        unknown_symptoms = []
        for symptom in input_data.symptoms:
            if symptom in utils.friendly_to_dataset:
                dataset_symptoms.append(utils.friendly_to_dataset[symptom])
            else:
        ...
        predicted_disease = utils.get_predicted_value(dataset_symptoms)
